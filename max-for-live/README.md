...
3. Create a new folder called: `Producer`
4. Copy the file `producer.js` into this folder
...
3. Make sure all 4 files are in the same folder:
   - `Producer.amxd`
   - `producer.js`
   - `get_context.js`
   - `write_midi.js`
...
3. Drag `Producer.amxd` onto any MIDI track
4. The device will appear at the bottom of Ableton
5. Paste your Anthropic API key (`sk-ant-...`) into the key field
...
- **Script not loading**: make sure `producer.js` is in the exact same folder as the `.amxd` file
